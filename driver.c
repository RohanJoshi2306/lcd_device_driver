...
/* Meta Information */
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Rohan Joshi");
MODULE_DESCRIPTION("LCD_Driver");
...
#define DRIVER_CLASS "driverModule"

/* LCD char buffer */
static char lcd_buffer[17];

/* Pinout for LCD Display */
unsigned int gpios[10] = {
	2, /* Enable Pin */
...
void lcd_command(uint8_t data) {
 	gpio_set_value(RS, 0);	/* RS to Instruction */
	lcd_send_byte(data);
}
...
void lcd_data(uint8_t data) {
 	gpio_set_value(RS, 1);	/* RS to data */
	lcd_send_byte(data);
}
